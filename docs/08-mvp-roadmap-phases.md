...
| ~~**7. Beta hardening**~~ ✅ built | Playwright on the three critical flows (log-a-day, submit-recipe, follow-and-feed — caught and fixed two real submission bugs); PWA manifest + icons + offline-shell service worker (network-first navigations, cached fallback, prod-only registration); no-scale mode suppresses all weight UI on Progress (docs/07 §4); page-level `requireUser` guard (layouts and pages render in parallel in Next 15); streak query bounded; seed covers every vertical | Public beta |
