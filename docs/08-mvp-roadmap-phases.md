# MacroVerse — MVP, Roadmap, Development Phases, Module Layout & Simplicity Rules
...
| ~~**4. Restaurants + map + progress**~~ ✅ built | Guest mode; Leaflet/OSM/Nominatim map + "Around me" ranked cross-chain list; buildable-item plate builder + combo-meal recommendation; go-to orders; progress dashboard (weight/measurements/photos) + habits tracker; personal ingredient library; favorites/frequents split on Add Food; admin CSV import with validation/changelog; feedback mechanism; fallback dataset | Open the app near a real address, get a ranked nearby recommendation, build and log a bowl, see it reflected in progress |
| ~~**5. Workouts**~~ ✅ built | Workout logger + PRs (e1RM/volume/reps, detected on session finish, share offered not forced), community workouts + fork, official template shelf, grocery lists (recipe/plan → deduped, sectioned list), meal prep plans (composed from recipes, boards as filters) | Both remaining loggable content verticals live |
| ~~**6. Community + safety**~~ ✅ built | Groups (public, group feed, join/leave), challenges + auto-scoring (logged/protein days, workouts, custom check-in — computed live from logs), reports pipeline (structured reasons, safety-first queue, 3-report auto-hide, macro-confidence penalty) + admin queue with immutable audit log, per-user rate limits, content warning labels | Safe to open to strangers |
| ~~**7. Production hardening**~~ ✅ built | Playwright on the three critical flows (log-a-day, submit-recipe, follow-and-feed — caught and fixed two real submission bugs); PWA manifest + icons + offline-shell service worker (network-first navigations, cached fallback, prod-only registration); no-scale mode suppresses all weight UI on Progress (docs/07 §4); page-level `requireUser` guard (layouts and pages render in parallel in Next 15); streak query bounded; seed covers every vertical | Stable production |
