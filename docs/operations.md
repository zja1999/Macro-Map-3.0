...
| `NEXT_PUBLIC_MACROTRAY_DOWNLOAD_URL` | Windows installer CTA | Banner is not rendered |
| `NEXT_PUBLIC_MACROTRAY_TESTING_BUILD` | Set to `true` only for an unsigned trusted-tester release | Banner clearly identifies the download as a testing build |
...
For a small trusted-tester group before signing is configured, build an unsigned installer locally with `npm run tauri:build`, upload it as a GitHub pre-release, and set `NEXT_PUBLIC_MACROTRAY_DOWNLOAD_URL` to its direct asset URL together with `NEXT_PUBLIC_MACROTRAY_TESTING_BUILD=true`. The banner then warns about the expected Windows `Unknown publisher` prompt. Do not treat this testing path as a public release or enable automatic updates for it.
