...
- MacroTray public signed download remains gated on Windows Authenticode certificate secrets, Tauri updater key material, a public signed GitHub Release, and clean-machine smoke tests. A clearly labelled, manually updated unsigned GitHub pre-release may be shared with a small trusted tester group. macOS/Linux packages are deferred.
...
On **2026-07-13**, interaction-ranked follow suggestions and shared username autocomplete passed `node node_modules/typescript/bin/tsc --noEmit`, `npm run test:security` (11/11), `npm run build` (59 generated routes/pages), `git diff --check`, and the focused `tests/e2e/user-suggestions.spec.ts` browser flow (1/1). A direct local PGlite query returned `prep_king`, `demo`, then `macroverse_admin` for `coach_dan` based on stored engagement while the fully-followed demo account correctly had no suggestion fallback. The browser test confirmed a case-insensitive prefix request, profile-context dropdown, and selection into the group-invite username field. No schema change or production deployment was performed for this feature.

On **2026-07-13**, the username/password and Google-recovery change passed `node node_modules/typescript/bin/tsc --noEmit`, `npm run test:security` (11/11), `npm run db:push` against local PGlite, `npm run build` (58 generated routes/pages), `git diff --check`, and all 5 focused assertions in `tests/e2e/auth-methods.spec.ts`, including a real seeded username login. The Playwright-managed Windows server reported all assertions passed but did not self-exit, so its verified process tree was stopped before later database work. No demo seed command ran. The production duplicate-OAuth preflight then passed, `drizzle-kit push` applied the additive schema to Neon, and the deployed `/api/auth/google/start?purpose=sign_in` route returned `307` to Google with the production callback URI instead of its prior `500`. Real-provider callback, account setup, linking, recovery, and reauthentication smoke tests remain externally gated.

On **2026-07-13**, the MacroTray change passed `npx tsc --noEmit`, `npm run test:security` (7/7), the two focused tests in `tests/e2e/macrotray.spec.ts`, `git diff --check`, `npm run build` (57 static pages), and `npm run db:push` against the local PGlite database while no development server was running. Tauri environment inspection found WebView2. Rust/Cargo and the Visual Studio C++/Windows SDK are installed but were absent from Codex's inherited `PATH`; an explicitly configured native `npm run tauri:build` completed and produced the unsigned per-user NSIS installer. The signed installer/updater, clean-machine test, and production pairing smoke test remain externally gated.
