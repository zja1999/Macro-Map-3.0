...
import { getDayLogs, getSavedRecipes, getFrequents } from "@/lib/queries";
...
export default async function AddFoodPage({
  ...
  // multi-add tally (plan §4.1): running slot + day totals under the whole flow
  const { logs: dayLogs } = await getDayLogs(user.id, date);
  const slotKcal = Math.round(dayLogs.filter((l) => l.mealSlot === slot).reduce((a, l) => a + l.calories, 0));
  const dayKcal = Math.round(dayLogs.reduce((a, l) => a + l.calories, 0));
  const remaining = user.targets ? user.targets.calories - dayKcal : null;
  ...
  return (
    <div className="mx-auto max-w-xl space-y-4 pb-24">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/track?date=${date}`} className="text-xs font-semibold text-text-tertiary transition hover:text-text">
          Done
        </Link>
      </div>
      ...
      <div className="flex gap-1.5">
        {MEAL_SLOTS.map((s) => (
          <Link
            ...
            className={`flex-1 rounded-full border px-2 py-1.5 text-center text-xs font-semibold capitalize transition ${
              s === slot
      ...
      <div className="flex gap-1 rounded-full border border-border bg-surface-1 p-1">
        {[
          ...
          <Link
            ...
            className={`flex-1 rounded-full px-2 py-1.5 text-center text-xs font-semibold transition ${
              tab === t.key ? "bg-accent text-black" : "text-text-secondary hover:text-text"
            }`}
          >
            {t.label}
          </Link>
      ...
      {tab === "search" && (
        <>
          {/* search field with the barcode overlay trigger inside */}
              ...
              <div className="flex flex-wrap gap-1.5">
                {frequents.map((f) => (
                  <form key={f.name} action={quickAdd}>
                    ...
                    <button
                      className="flex items-center gap-1.5 rounded-full border border-border bg-surface-2 px-2.5 py-1 text-xs text-text-secondary transition hover:border-accent hover:text-accent"
                      title={`${Math.round(Number(f.calories))} kcal · ${Math.round(Number(f.proteinG))}g P — logged ${f.count}× recently`}
                    >
                      <RotateCcw size={12} /> {f.name.length > 32 ? `${f.name.slice(0, 32)}…` : f.name}
                    </button>
                  </form>
                ))}
              </div>
                ...
                {fallbackResults.map((f) => (
                  <Card key={f.id} className="flex items-center justify-between gap-3 p-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-0.5 text-[10px] text-text-tertiary">per {f.servingDesc}</div>
                    </div>
          ...
          <div className="space-y-2">
            ...
            {q && results.length === 0 && fallbackResults.length === 0 && (
              <p className="py-6 text-center text-sm text-text-tertiary">No foods match “{q}” — try Quick add.</p>
            )}
          </div>
        </>
      )}
      ...
      {tab === "saved" && (
        <div className="space-y-2">
          {(usuals.menuItems.length > 0 || usuals.orders.length > 0) && (
            <>
              <h2 className="text-micro text-text-tertiary">My usual</h2>
          ...
          {myOrders.length > 0 && (
            <>
              ...
              <h2 className="text-micro pt-2 text-text-tertiary">Saved recipes</h2>
            </>
          )}
          ...
          <form action={quickAdd} className="space-y-3">
            ...
            <div className="grid grid-cols-4 gap-2">
              {(
                ...
                <label key={name} className="space-y-1 text-[10px] text-text-secondary">
                  {label}
      ...
      {/* multi-add running tally (plan §4.1) — pinned above the tab bar */}
      <div className="fixed inset-x-0 bottom-16 z-30 mb-safe md:bottom-4">
        <div className="mx-auto flex max-w-xl items-center justify-between gap-3 border-t border-border bg-surface-1/95 px-4 py-2.5 backdrop-blur md:rounded-xl md:border">
          <div className="min-w-0 text-xs text-text-secondary">
            <span className="font-semibold capitalize text-text">{slot}</span>{" "}
            <span className="tabular-nums">{slotKcal} kcal</span>
            {remaining != null && (
              <>
                {" · "}
                <span className={`font-semibold tabular-nums ${remaining < 0 ? "text-danger" : "text-accent"}`}>
                  {Math.abs(remaining)}
                </span>{" "}
                {remaining < 0 ? "kcal over today" : "kcal left today"}
              </>
            )}
          </div>
          <Link
            href={`/track?date=${date}`}
            className="flex shrink-0 items-center gap-1 rounded-lg bg-accent px-3 py-1.5 text-xs font-bold text-black transition hover:brightness-110"
          >
